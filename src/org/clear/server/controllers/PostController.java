...
public class PostController {
	...
	// show the content of the post
	@RequestMapping(value = "/data/posts/show/friend", method = RequestMethod.GET)
	public String showFriendPost(@RequestParam("user") String user, Model model) {

		PersistenceManager pm = PMF.get().getPersistenceManager();

		Friends friends = Friends.byUser(user, pm);
		if (friends != null) {
			Set<Post> set = Post.getPostbyFriend(friends.getFriendIds(), pm);
			for(Post p:set){
