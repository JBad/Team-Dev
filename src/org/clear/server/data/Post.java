...
public class Post {
	...
	// Every persistent object needs a primary key.
		...
		public static Set<Post> getPostbyFriend(Set<String> friends, PersistenceManager pm){
			Set<Post> sets = new HashSet<Post>();
			Query query = pm.newQuery("select from " + Post.class.getName());
			List<Post> allPost = (List<Post>) query.execute();
			for(Post p: allPost){
				for(String f:friends){
					if(p.getList().contains(f)){
						sets.add(p);
						break;
					}
				}
